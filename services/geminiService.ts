...
- You can answer questions about:
  - **Customers**: Details, policies, notes, financials.
  - **Leads**: Status, follow-up dates, notes.
  - **Tasks**: Pending items, due dates, assignees.
  - **Finances**: Recent expenses, income, commissions.
- Be specific. If asked about expenses, list the recent ones with amount and description.
- If asked about tasks, mention what they are and when they are due.
- Provide human-like, conversational answers.
...
export const generateUpsellSuggestion = async (
    ...
    addToast?: (message: string, type?: 'success' | 'error') => void
): Promise<{ suggestion: string; pitch: string } | null> => {
    if (!process.env.API_KEY) {
        if (addToast) addToast("Using fallback for AI suggestion.", "error");
    ...
    try {
        ...
        for (const policy of member.policies) {
            if (policy.insuranceTypeId) {
                for (const category of upsellCategories) {
                    if (category.linkedInsuranceTypeIds.includes(policy.insuranceTypeId)) {
        ...
        const ownedCategories = upsellCategories.filter(uc => ownedCategoryIds.has(uc.id)).map(uc => uc.name);
        const availableCategories = upsellCategories.filter(uc => !ownedCategoryIds.has(uc.id)).map(uc => uc.name);

        if (availableCategories.length === 0) {
            return { suggestion: "Fully Covered", pitch: "This customer has policies in all available categories. Focus on service and periodic reviews." };
        }
        
        const memberProfileForPrompt = {
            name: member.name,
            age: new Date().getFullYear() - new Date(member.dob).getFullYear(),
            maritalStatus: member.maritalStatus,
            city: member.city,
            isFamily: member.isSPOC || member.spocId,
        };

        const prompt = `
You are an expert sales strategist for a financial advisor. Your task is to analyze a customer's profile and recommend the next best product to upsell or cross-sell.

**Customer Profile:**
${JSON.stringify(memberProfileForPrompt)}

**Existing Products Owned by Customer:**
${JSON.stringify(ownedCategories)}

**Available Products to Suggest:**
${JSON.stringify(availableCategories)}

**Instructions:**
1. Analyze the customer's profile and their existing products.
2. From the "Available Products" list, identify the single most logical and valuable product to recommend next.
3. Create a brief, compelling, and personalized sales pitch (2-3 sentences) for the advisor to use when contacting the customer. The pitch should be empathetic and highlight a key benefit.
4. Return a valid JSON object with two keys: "suggestion" (the name of the suggested product) and "pitch" (your sales pitch).

Your response MUST be a valid JSON object. Do not include any other text or explanation.
`;
        ...
        if (result && result.suggestion && result.pitch) {
            return result;
        }
