...
const ToggleSwitch = ({ label, enabled, onChange, disabled }: {label: string, enabled: boolean, onChange: () => void, disabled?: boolean}) => (
    <div className="flex items-center">
        <span className={`text-sm font-medium ${disabled ? 'text-gray-400 dark:text-gray-500' : 'text-gray-700 dark:text-gray-300'} mr-3`}>{label}</span>
        <button
            type="button"
            className={`relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-primary ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="flex items-end">
            <ToggleSwitch 
                ...
                enabled={!!data.automatedGreetingsEnabled} 
                onChange={() => onChange('automatedGreetingsEnabled', !data.automatedGreetingsEnabled)}
                disabled={!canModify}
            />
          </div>
