...
import { Bot, Send, Plus, Trash2, Edit2, Save, XCircle, MessageCircle, Tv, TestTube2, MessageSquare, Briefcase, RefreshCw } from 'lucide-react';
import Button from './ui/Button.tsx';
import { Member, Lead, Task, Expense, ManualIncome, ManualCommission } from '../types.ts';
...
const Chatbot: React.FC<ChatbotProps> = ({ members, leads, tasks, expenses, manualIncomes, manualCommissions, addToast }) => {
  ...
  const handleAdminSendMessage = async (text: string) => {
    ...
    const botText = await getChatbotResponse(text, context, addToast);
  ...
  const handleNewChat = () => {
    setAssistantMessages([assistantInitialMsg]);
            ...
            <div className="animate-fade-in">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-gray-800 dark:text-white">AI Chatbot Assistant</h3>
                      <p className="text-sm text-gray-500 dark:text-gray-400">Your internal tool for quick customer insights and actions.</p>
                    </div>
                  </div>
                  <Button variant="light" size="small" onClick={handleNewChat}><RefreshCw size={14} className="mr-2"/> New Chat</Button>
                </div>
                <ChatWindow messages={assistantMessages} onSendMessage={handleAdminSendMessage} onSuggestionClick={handleAdminSendMessage} isTyping={isAssistantTyping} input={assistantInput} setInput={setAssistantInput} placeholder="Ask about customers, policies, income, expenses..." initialSuggestions={assistantInitialSuggestions} />
            </div>
        )}
