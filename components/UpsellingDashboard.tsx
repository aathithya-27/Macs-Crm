...
const FILTER_OPTIONS_CONFIG = [
    ...
    { key: 'ownership', label: 'Product Status' },
...
const BulkCreationModal: React.FC<BulkCreationModalProps> = ({
    ...
    const handleSubmit = () => {
        ...
        const upsellingSource = leadSources.find(ls => ls.name === 'Upselling');
        const sourceIdToUse = upsellingSource ? upsellingSource.id : null; 
            ...
            <div className="p-6">
                ...
                <div className="space-y-4">
                    ...
                    <div className="col-span-2">
                            ...
                            rows={2}
                            placeholder="E.g. Diwali Upselling Campaign..."
                            className="w-full rounded-md border border-gray-300 py-2 px-3 focus:outline-none focus:ring-brand-primary dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                            />
                    </div>
                </div>
...
const UpsellingDashboard: React.FC<UpsellingDashboardProps> = ({ 
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border dark:border-gray-700 p-4">
                ...
                {visibleFilters.length > 0 && (
                    <div className="mt-4 pt-4 border-t dark:border-gray-700 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 animate-fadeIn">
                        ...
                        {visibleFilters.includes('ownership') && <SearchableSelect label="Product Status" options={[{value:'all', label:'All'}, {value:'owned', label:'Owned'}, {value:'not_owned', label:'Not Owned'}]} value={filters.ownership} onChange={(v) => setFilters(p => ({...p, ownership: v}))} />}
                                ...
                                {showMutualFundsColumn && (
                                    <th className="px-4 py-3 text-center font-semibold text-gray-600 dark:text-gray-300">
                                        <div className="flex flex-col items-center">
                                            ...
                                            <span className="text-xs font-normal text-gray-500 mt-0.5 bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full">
                                ...
                                return (
                                    <tr key={member.id} className={`hover:bg-gray-50 dark:hover:bg-gray-700/50 ${isSelected ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
                                        ...
                                        <td className="px-4 py-3">
                                            ...
                                            <div className="text-xs text-gray-400 mt-0.5">{member.city}, {member.state}</div>
                                        </td>
                                        ...
                                        {showMutualFundsColumn && (
                                            <td className="px-4 py-3 text-center">
                                                {hasMF ? (
                                                    ...
                                                    <button onClick={() => onCreateLead(member, 'Mutual Funds')} className="text-xs text-blue-600 border border-blue-200 bg-blue-50 px-2 py-1 rounded">Pitch MF</button>
                                                )}
                                            </td>
                                        )}
                                            ...
                                            return (
                                                <td key={cat.id} className="px-4 py-3 text-center">
                                                    {hasPolicy ? (
                                                        ...
                                                        <button onClick={() => onCreateLead(member, cat.name)} className="text-xs text-blue-600 border border-blue-200 bg-blue-50 px-2 py-1 rounded">Pitch {cat.name.split(' ')[0]}</button>
                                                    )}
                                                </td>
                                            );
                                        })}
