...
const CustomMessageScheduler: React.FC<{ members: Member[]; onSchedule: (message: Omit<CustomScheduledMessage, 'id'>) => void; addToast: (message: string, type?: 'success' | 'error') => void; disabled: boolean; }> = ({ members, onSchedule, addToast, disabled }) => {
...
const AppointmentScheduler: React.FC<{ members: Member[]; onSchedule: (details: { memberId: string, dateTime: string }) => void; addToast: (message: string, type?: 'success' | 'error') => void; disabled: boolean; }> = ({ members, onSchedule, addToast, disabled }) => {
    const [memberId, setMemberId] = useState<string>('');
    const [dateTime, setDateTime] = useState('');
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <Input type="datetime-local" id="appt-datetime" value={dateTime} onChange={(e) => setDateTime(e.target.value)} disabled={disabled} />
                    </div>
                </div>
                <div className="text-right">
                    <Button type="submit" variant="primary" disabled={disabled}><CalendarIcon size={16} /> Schedule Appointment</Button>
                </div>
            </form>
        </div>
    );
                ...
                <div className="w-full md:w-auto flex-shrink-0 flex items-start justify-end gap-2 flex-wrap">
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        : isRenewal && n.policy ? (<>
                            ...
                            <Button variant="primary" size="small" onClick={() => handleRenew(n.member.id, n.policy!.id, n.id)} disabled={!canModifyPolicies} title={!canModifyPolicies ? "You do not have permission to renew policies" : ""}>
                                <Check size={14} /> Mark as Renewed
                            </Button>
                        </>) : null}
                    </div>
                    <div className="border-l border-gray-200 dark:border-gray-600 pl-2 ml-1">
                         <Button variant="light" size="small" className="!p-1.5" onClick={() => onDismissItem(n.id)} aria-label="Dismiss notification">
                            <X size={14} />
                        </Button>
                    </div>
                </div>
                    ...
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border dark:border-gray-700">
                        ...
                        <div className="space-y-4">
                                    ...
                                    <div key={task.id} className="p-3 border-b border-gray-200 dark:border-gray-700 last:border-b-0">
                                        <div className="flex items-start justify-between">
                                            ...
                                            <div className="flex items-center gap-2 flex-shrink-0">
                                                ...
                                                <Button size="small" variant="light" className="!p-1" onClick={() => onDismissItem(task.id)} aria-label="Dismiss task">
                                                    <X size={12}/>
                                                </Button>
                                            </div>
                                        </div>
                            ...
                            <div><h4 className="font-medium text-gray-700 dark:text-gray-300 mb-2 mt-4">Appointments</h4>
                                {appointments.length > 0 ? appointments.map(appt => (<div key={appt.id} className="flex items-center justify-between p-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded"><div><p className="text-sm text-gray-800 dark:text-gray-200">{appt.memberName}</p><p className="text-xs text-gray-500 dark:text-gray-400">{new Date(appt.dateTime).toLocaleString('en-GB')}</p></div><Button size="small" variant="danger" onClick={() => onDismissItem(appt.id)}><Trash2 size={12}/></Button></div>)) : (<div className="text-center py-4 text-sm text-gray-500 dark:text-gray-400"><CalendarIcon size={24} className="mx-auto text-gray-300 dark:text-gray-500" /><p className="mt-1 font-medium">No upcoming appointments.</p></div>)}
                            </div>
                        </div>
                    </div>
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border dark:border-gray-700">
                        ...
                        <ul className="space-y-3">{activityLog.length > 0 ? activityLog.map(log => (
                            <li key={log.id} className="flex items-start justify-between gap-3 text-sm">
                                ...
                                <Button size="small" variant="light" className="!p-1 flex-shrink-0" onClick={() => onDismissItem(log.id)} aria-label="Dismiss activity">
                                    <X size={12}/>
                                </Button>
                            </li>
                        )) : (<li className="text-center py-10 text-sm text-gray-500 dark:text-gray-400"><History size={24} className="mx-auto text-gray-300 dark:text-gray-500" /><p className="mt-1 font-medium">No recent activity.</p></li>)}</ul>
                    </div>
                </div>
            </div>}
                ...
                <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border dark:border-gray-700">
                    ...
                    <div className="space-y-6">
                        {rules.map(rule => (<div key={rule.id} className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg border dark:border-gray-600">
                            ...
                            </div><div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700/50 flex justify-end gap-2">{editingRuleId === rule.id ? (<><Button variant="secondary" size="small" onClick={handleCancelClick}><X size={14}/> Cancel</Button><Button variant="success" size="small" onClick={() => handleSaveClick(rule.id)}><Save size={14}/> Save Changes</Button></>) : (<Button variant="light" size="small" onClick={() => handleEditClick(rule)} disabled={!rule.enabled || !canModifyAutomation}><Edit2 size={14}/> Edit Rule</Button>)}</div>
                        </div>))}
                    </div>
                </div>
