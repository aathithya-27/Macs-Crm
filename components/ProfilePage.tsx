...
const ProfilePage: React.FC<ProfilePageProps> = ({ 
  ...
  const handleSave = () => { if (formData) onUpdateProfile(formData, false); };
  ...
  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0] && formData) {
      ...
      const updatedUser = { ...formData, profile: { ...formData.profile, photoUrl: newUrl } as EmployeeProfile };
  ...
  const TABS_CONFIG = [
      ...
      ...(isCurrentUserAdvisor ? [{ name: EmployeeModalTab.Customers, icon: <Users size={16}/> }] : []),
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border dark:border-gray-700">
        <div className="flex justify-between items-center p-6">
            <div className="flex items-center gap-4">
                 <div className="relative flex-shrink-0">
                    ...
                    <label htmlFor="profile-photo-upload" className="absolute -bottom-1 -right-1 bg-white dark:bg-gray-600 rounded-full p-1.5 cursor-pointer shadow-md hover:bg-gray-100 dark:hover:bg-gray-500 border dark:border-gray-500">
                ...
                <div>
                    ...
                    <p className="mt-2 text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-200 inline-block px-2 py-1 rounded-full">
                        {designationMap.get(formData.designationId) || 'Employee'}
                    </p>
                </div>
            </div>
        ...
        <div className="flex-shrink-0 px-6 border-t border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-t-lg">
